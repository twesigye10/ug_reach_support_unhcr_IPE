...
loc_tool <- "inputs/REACH DataPWD/Questions and Responses CODES.xlsx"
# question/choices codes and labels
df_questions <- readxl::read_excel(loc_tool, sheet = "Sheet1") %>% 
...
df_choices <- readxl::read_excel(loc_tool, sheet = "Sheet1") %>% 
...
df_tool_select_type <- df_questions %>% 
...
# read analysis
df_unformatted_analysis <- readxl::read_excel("outputs/analysis_ipe_verification_filtered.xlsx", sheet = "HH level analysis") %>% 
  mutate(analysis_choice_id = analysis_var_value,
         analysis_var_value_label = ifelse(analysis_choice_id %in% df_choices_support$choice_code, recode(analysis_choice_id, !!!setNames(df_choices_support$choice_label, df_choices_support$choice_code)), analysis_var_value),
...
# openxlsx::write.xlsx(df_analysis_wide, "outputs/prepare_4_column_ordering_ipe_verif_hh.xlsx", overwrite = T)

df_cols_for_ordering <- readxl::read_excel("outputs/column_ordering_ipe_verif_hh.xlsx", sheet = "combined")
...
addWorksheet(wb, sheetName="HH analysis - IPE Verification")

# header showing results headings
writeData(wb, sheet = "HH analysis - IPE Verification", df_extracted_header %>% head(1), startCol = 2, 
          startRow = 1, headerStyle = hs2, colNames = FALSE, 
          borders = "all", borderColour = "#000000", borderStyle = "thin")

setColWidths(wb = wb, sheet = "HH analysis - IPE Verification", cols = 1, widths = 70)
setColWidths(wb = wb, sheet = "HH analysis - IPE Verification", cols = 2, widths = 16)
setColWidths(wb = wb, sheet = "HH analysis - IPE Verification", cols = 3:4, widths = 10)
setColWidths(wb = wb, sheet = "HH analysis - IPE Verification", cols = 5:80, widths = 10)
setColWidths(wb = wb, sheet = "HH analysis - IPE Verification", cols = 81, widths = 20)
...
for (i in 1:length(sheet_variables_data)) {
  ...
  writeData(wb, sheet = "HH analysis - IPE Verification", get_question_label, startCol = 1, startRow = previous_row_end + 1)
  writeData(wb, sheet = "HH analysis - IPE Verification", get_qn_type, startCol = 2, startRow = previous_row_end + 1)
  addStyle(wb, sheet = "HH analysis - IPE Verification", hs2, rows = previous_row_end + 1, cols = 1:2, gridExpand = TRUE)
  ...
  writeData(wb = wb, 
            sheet = "HH analysis - IPE Verification", 
            x = current_variable_data %>% 
              select(-c(analysis_var, analysis_var_value,
...
# freeze pane
freezePane(wb, "HH analysis - IPE Verification", firstActiveRow = 2, firstActiveCol = 3)

# openXL(wb)

# analysis formatting individual level ------------------------------------

# read analysis
df_unformatted_analysis_individual <- readxl::read_excel("outputs/analysis_ipe_verification_filtered.xlsx", sheet = "Individual level analysis") %>% 
  mutate(analysis_choice_id = analysis_var_value,
         analysis_var_value_label = ifelse(analysis_choice_id %in% df_choices_support$choice_code, recode(analysis_choice_id, !!!setNames(df_choices_support$choice_label, df_choices_support$choice_code)), analysis_var_value),
         Indicator = ifelse(analysis_var %in% df_extra_analysis_info$extra_info_code, recode(analysis_var, !!!setNames(df_extra_analysis_info$indicator, df_extra_analysis_info$extra_info_code)), analysis_var),
         Sector = ifelse(analysis_var %in% df_extra_analysis_info$extra_info_code, recode(analysis_var, !!!setNames(df_extra_analysis_info$sector, df_extra_analysis_info$extra_info_code)), NA_character_),
...
# openxlsx::write.xlsx(df_analysis_wide_individual, "outputs/prepare_4_column_ordering_ipe_verif_individual.xlsx", overwrite = T)

df_cols_for_ordering_individual <- readxl::read_excel("outputs/column_ordering_ipe_verif_individual.xlsx", sheet = "combined")

reordered_columns_individual <- df_cols_for_ordering_individual %>%
  pivot_longer(cols = c(result_col, n_unweighted), names_to = "entries", values_to = "columns") %>%
  pull(columns)

# reorder
extra_cols_for_analysis_tables_individual <- c("Indicator")
df_analysis_wide_individual_reodered <- df_analysis_wide_individual %>%
  # relocate(any_of(extra_cols_for_analysis_tables), .before = "analysis_var") %>% 
  relocate("analysis_var_value_label", .after = "analysis_var_value") %>% 
  relocate(any_of(reordered_columns_individual), .after = "analysis_var_value_label") %>% 
  relocate(analysis_type, .after = "analysis_var_value_label")

cols_for_num_pct_formatting_individual <- df_analysis_wide_individual_reodered %>% 
  # select(stat_total:row_id, - c("Indicator", "Sector")) %>% 
  select(stat_total:row_id, - c("Indicator", "Sector")) %>% 
  select(!matches("^n_"), -row_id) %>% 
  colnames()
...
df_to_extract_header_individual = df_analysis_wide_individual_reodered %>% 
  select(-any_of(c("analysis_var", "analysis_var_value", "analysis_var_value_label",
                   "int.analysis_var", "analysis_choice_id", "Indicator", "row_id"))) %>% 
  colnames()
...
# add sheet for the individual analysis
addWorksheet(wb, sheetName="Individual analysis - IPE Verif")

# header showing results headings
writeData(wb, sheet = "Individual analysis - IPE Verif", df_extracted_header_individual %>% head(1), startCol = 2, 
          startRow = 1, headerStyle = hs2, colNames = FALSE, 
          borders = "all", borderColour = "#000000", borderStyle = "thin")

setColWidths(wb = wb, sheet = "Individual analysis - IPE Verif", cols = 1, widths = 70)
setColWidths(wb = wb, sheet = "Individual analysis - IPE Verif", cols = 2, widths = 16)
setColWidths(wb = wb, sheet = "Individual analysis - IPE Verif", cols = 3:4, widths = 10)
setColWidths(wb = wb, sheet = "Individual analysis - IPE Verif", cols = 5:80, widths = 10)
setColWidths(wb = wb, sheet = "Individual analysis - IPE Verif", cols = 81, widths = 20)

# split variables to be written in different tables with in a sheet
sheet_variables_data_individual <- split(df_analysis_wide_individual_reodered, factor(df_analysis_wide_individual_reodered$analysis_var, levels = unique(df_analysis_wide_individual_reodered$analysis_var)))
...
for (i in 1:length(sheet_variables_data_individual)) {
  ...
  get_question <- current_variable_data_individual %>% select(analysis_var) %>% unique() %>% pull()
  get_question_label <- current_variable_data_individual %>% select(Indicator) %>% unique() %>% pull()
  get_qn_type <- current_variable_data_individual %>% select(analysis_type) %>% unique() %>% pull()
  ...
  # print(paste0("current start row: ", current_row_start_individual, ", variable: ", get_question))
  
  # add header for variable
  writeData(wb, sheet = "Individual analysis - IPE Verif", get_question_label, startCol = 1, startRow = previous_row_end_individual + 1)
  writeData(wb, sheet = "Individual analysis - IPE Verif", get_qn_type, startCol = 2, startRow = previous_row_end_individual + 1)
  addStyle(wb, sheet = "Individual analysis - IPE Verif", hs2, rows = previous_row_end_individual + 1, cols = 1:2, gridExpand = TRUE)
  ...
  print(paste0("start row: ", current_row_start_individual, ", previous end: ", previous_row_end_individual, ", data length: ", current_data_length_individual, ", variable: ", get_question))
  
  writeData(wb = wb, 
            sheet = "Individual analysis - IPE Verif", 
...
# freeze pane
freezePane(wb, "Individual analysis - IPE Verif", firstActiveRow = 2, firstActiveCol = 3)

# openXL(wb)

# outputting formatted analysis -------------------------------------------

saveWorkbook(wb, paste0("outputs/", butteR::date_file_prefix(),"_hh_verification_filtered_IPE_formatted_analysis.xlsx"), overwrite = TRUE)
openXL(file = paste0("outputs/", butteR::date_file_prefix(),"_hh_verification_filtered_IPE_formatted_analysis.xlsx"))

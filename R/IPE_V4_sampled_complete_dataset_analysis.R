...
# clean data
sampled_not_filtered_data_path <- "inputs/ipe_hh_sampled_not_filtered_data_with_composites.xlsx"

not_filtered_data_nms <- names(readxl::read_excel(path = sampled_not_filtered_data_path, n_max = 2000, sheet = "sampled_hh_data"))
not_filtered_c_types <- ifelse(str_detect(string = not_filtered_data_nms, pattern = "_other$"), "text", "guess")

df_sample_not_filtered_with_composites <- readxl::read_excel(path = sampled_not_filtered_data_path, sheet = "sampled_hh_data", col_types = not_filtered_c_types, na = "NA") %>% 
...
df_sample_mental_health_with_composites <- readxl::read_excel(path = sampled_not_filtered_data_path, sheet = "sampled_individual_data", na = "NA")
...
# refugee weights
ref_weight_table <- make_refugee_weight_table(input_df_ref = df_sample_not_filtered_with_composites, 
...
df_ref_with_weights <- df_sample_not_filtered_with_composites %>% 
...
openxlsx::write.xlsx(sample_analysis_out_list, paste0("outputs/", butteR::date_file_prefix(), "_analysis_ipe_hh_sampled_complete_dataset.xlsx"))
openxlsx::write.xlsx(sample_analysis_out_list, paste0("outputs/analysis_ipe_hh_sampled_complete_dataset.xlsx"))

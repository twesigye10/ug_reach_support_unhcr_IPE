...
create_composites_sampled_specific_dates <- function(input_df) {
  input_df %>% 
    ...
    mutate(i.fcs = (cereal_grain_root_fcs*2 + pulses_fcs*3 + vegetables_fcs*1 +  
           ...
           i.hh_size_jerrycan_category = case_when(hh_size <= 3 & jerry_can_cond %in% c("good", "moderate") ~ "between_1_and_3_HH_size",
           ...
           i.number_water_container = ifelse(!is.na(i.hh_size_jerrycan_category), as.numeric(jerry_can_num), NA_integer_),
           i.hh_size_tarpaulin_category = case_when(hh_size <= 3 & tarpaulin_cond %in% c("good", "moderate") ~ "between_1_and_3_HH_size",
           ...
           i.number_tarpaulin = ifelse(!is.na(i.hh_size_tarpaulin_category), as.numeric(tarpaulin_num), NA_integer_),
           i.hh_size_plastic_bucket_category = case_when(hh_size <= 3 & plastic_bucket_cond %in% c("good", "moderate") ~ "between_1_and_3_HH_size",
           ...
           i.number_plastic_bucket = ifelse(!is.na(i.hh_size_plastic_bucket_category), as.numeric(plastic_bucket_num), NA_integer_),
           i.hh_size_solar_lamp_category = case_when(hh_size <= 3 & solar_lamp_cond %in%c("good", "moderate") ~ "between_1_and_3_HH_size",
           ...
           i.number_solar_lamp = ifelse(!is.na(i.hh_size_solar_lamp_category), as.numeric(solar_lamp_num), NA_character_),
           i.hh_size_kitchen_set_category = case_when(hh_size <= 3 & kitchen_set_cond %in% c("good", "moderate") ~ "between_1_and_3_HH_size",
           ...
           i.number_kitchen_set = ifelse(!is.na(i.hh_size_kitchen_set_category), as.numeric(kitchen_set_num), NA_integer_),
